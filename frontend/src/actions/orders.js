...
import { CREATE_ORDER_ERROR, CREATE_ORDER_REQUEST, DELETE_CART, ORDER_DETAIlS, ORDER_DETAILS_FAIL, ORDER_DETAILS_REQURST, PAYMENT_METHOD } from "./types"
...
export const placeOrder = (order) => async (dispatch, getState) => {
   ...
   try {
      ...
      // const { data } = await eatsygreen.post('api/users/orders', order, {
      //    headers: {
...
export const getOrderDetails = (id) => async (dispatch, getState) => {
   ...
   try {
      const user = getState().user?.user;
      const { data } = await eatsygreen.get(`/api/users/orders/${id}`, {
         headers: {
            Authorization: `Bearer ${user.token}`
         }
      })
