...
export const siginUser = (email,password)=>async(dispatch,getState)=>{
   ...
   try {
       const {data}=await eatsygreen.post('/api/users/signin',{email,password})
...
export const signupUser =(name,email,password)=>async dispatch =>{
   ...
   try {
       const {data}=await eatsygreen.post('/api/users/signup',{name,email,password})
...
export const forgotPassword =(email)=>async dispatch =>{
   ...
   try {
       const {data}=await eatsygreen.post('/api/users/forgotpassword',{email})
...
export const resetPassword =(id, token, password)=>async dispatch =>{
   ...
   try {
       const {data}=await eatsygreen.post(`/api/users/resetPassword/${id}/${token}`,{password})
...
export const updateprofile=(userInfo)=>async (dispatch,getState)=>{
    ...
    try{
      ...
      const {data} = await eatsygreen.put('/api/users/updateProfile',userInfo,{
        headers:{
